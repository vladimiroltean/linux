...
static int ftdi_set_baudrate(struct usb_interface *intf, int baudrate)
{
	...
	if (ret < 0) {
		dev_err(&intf->dev, "failed to set baudrate: %d\n", ret);
		return ret;
	}
...
static int ftdi_set_latency(struct usb_interface *intf, int latency_msec)
{
	...
	ret = ftdi_ctrl_xfer(intf, &desc);
	if (ret < 0) {
		dev_err(&intf->dev, "failed to set latency: %d\n", ret);
		return ret;
	}
...
static int ftdi_disable_bitbang(struct usb_interface *intf)
{
	...
	if (ret < 0) {
		dev_err(&intf->dev, "disable bitbang failed: %d\n", ret);
		return ret;
	}
...
static int ftdi_read_eeprom(struct ft232h_intf_priv *priv)
{
	...
	for (i = 0; i < FTDI_MAX_EEPROM_SIZE / 2; i++) {
		...
		if (ret < 0) {
			dev_err(&priv->intf->dev, "EEPROM read failed: %d\n",
				ret);
			return ret;
		}
	}
				    ...
				    unsigned char *pins)
{
	...
	ret = ftdi_ctrl_xfer(priv->intf, &desc);
	if (ret < 0) {
		dev_err(chip->parent, "failed to get pin values: %d\n", ret);
		return ret;
	}
...
static int ftdi_cbus_gpio_set(struct gpio_chip *chip,
			      unsigned int offset, int value)
{
	...
	if (ret < 0)
		dev_err(chip->parent, "setting pin value failed: %d\n", ret);
...
#define work_to_priv(work) \
	container_of(work, struct ft232h_intf_priv, irq_work)

static void ftdix_read_gpios(struct work_struct *work)
{
	struct ft232h_intf_priv *priv = work_to_priv(work);
	uint8_t oldio;
	uint8_t new_value;
	unsigned long flags;
...
//	priv->mpsse_gpio.free   = NULL;          //16ton?
	...
	priv->mpsse_gpio.ngpio = ARRAY_SIZE(gpio_names);
	...
	if (ret < 0) {
		dev_err(dev, "Failed to set MPSSE mode: %pe\n", ERR_PTR(ret));
		return ret;
	}

	ret = devm_gpiochip_add_data(dev, &priv->mpsse_gpio, priv);
	if (ret < 0) {
		dev_err(dev, "Failed to add MPSSE GPIO chip: %pe\n", ERR_PTR(ret));
		return ret;
	}
...
#define gpiochip_to_priv(chip) \
	container_of(chip, struct ft232h_intf_priv, mpsse_gpio)

static int ftdi_mpsse_gpio_to_irq(struct gpio_chip *chip, unsigned offset)
{
	struct ft232h_intf_priv *priv = gpiochip_to_priv(chip);
...
mpsse_dev_register(struct ft232h_intf_priv *priv,
			     ...
			     const struct usb_device_id *id)
{
	...
	/* Our priv->udev->product == "Quad RS232-HS" */
