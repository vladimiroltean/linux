 ...
 */
static void lynx_28g_pll_ex_dly_clk_enable(struct lynx_pll *pll,
					   bool enable)
{
	u32 val = 0;
...
static void lynx_28g_pll_get_ex_dly_clk(struct lynx_pll *pll)
{
	spin_lock(&pll->lock);
...
static void lynx_28g_pll_put_ex_dly_clk(struct lynx_pll *pll)
{
	spin_lock(&pll->lock);
...
static void lynx_28g_pll_read_configuration(struct lynx_pll *pll)
{
	...
	if (ex_dly_sel) {
		dev_dbg(pll->priv->dev, "PLL%cCR1[EX_DLY_SEL] found set\n",
			pll->id == 0 ? 'F' : 'S');
