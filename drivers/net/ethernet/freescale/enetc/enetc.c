			 ...
			 struct enetc_tx_swbd *xdp_tx_arr, int num_tx_swbd)
{
	...
	if (enetc_bd_unused(tx_ring) < ENETC_TXBDS_NEEDED(num_tx_swbd))
		return false;

	while (!tmp_tx_swbd->is_eof) {
		tmp_tx_swbd++;
		   ...
		   struct xdp_frame **frames, u32 flags)
{
	...
	for (k = 0; k < num_frames; k++) {
		...
		if (!enetc_xdp_tx(tx_ring, xdp_redirect_arr, xdp_tx_bd_cnt)) {
	...
	if ((flags & XDP_XMIT_FLUSH) || (k != xdp_tx_frm_cnt))
		enetc_update_tx_ring_tail(tx_ring);
				   ...
				   struct bpf_prog *prog)
{
	...
	while (likely(rx_frm_cnt < work_limit)) {
		...
		case XDP_REDIRECT:
			...
			if (xdp_redirect_frm_cnt > ENETC_DEFAULT_TX_WORK) {
				xdp_do_flush_map();
