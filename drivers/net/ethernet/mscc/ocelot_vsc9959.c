...
static int felix_chip_init(struct ocelot *ocelot, const struct ocelot_ops *ops)
{
	int ret;

	ocelot->map = vsc9959_regmap;
	ocelot->stats_layout = vsc9959_stats_layout;
	ocelot->num_stats = ARRAY_SIZE(vsc9959_stats_layout);
	ocelot->num_mact_rows = 2048;
	ocelot->ops = ops;

	ret = ocelot_regfields_init(ocelot, vsc9959_regfields);
	if (ret) {
...
static irqreturn_t felix_irq_handler(int irq, void *arg)
{
	struct ocelot *ocelot = arg;
	...
	ocelot_xtr_irq_handler(irq, arg);
				  ...
				  struct device_node *ports)
{
	...
	for_each_available_child_of_node(ports, portnp) {
		...
		err = ocelot_probe_port(ocelot, port, target, phydev);
		if (err) {
			   ...
			   const struct pci_device_id *id)
{
	...
	err = felix_chip_init(ocelot, &vsc9959_ops);
	if (err)
		goto err_chip_init;
	...
	ocelot->vcap = vsc9959_vcap_props;
