					     ...
					     unsigned long ports)
{
	...
	/* The PGID index 0 corresponds to ENTRYTYPE_MACv4/ENTRYTYPE_MACv6
	 * entries and does not need to be programmed to hardware
	 */
	if (!index)
		return pgid;

	ocelot->pgid_mask[pgid->index] = true;
	ocelot_write_rix(ocelot, pgid->ports, ANA_PGID_PGID, pgid->index);
	dev_err(ocelot->dev, "PGID[%d]=0x%lx\n", pgid->index, pgid->ports);
...
static void ocelot_pgid_free(struct ocelot *ocelot, struct ocelot_pgid *pgid)
{
	...
	if (pgid->index)
		ocelot->pgid_mask[pgid->index] = false;
					       ...
					       const struct ocelot_multicast *mc)
{
	...
	list_for_each_entry(pgid, &ocelot->pgids, list) {
		/* Ignore the dummy PGIDs for MACv4/MACv6 entries */
			...
			const struct switchdev_obj_port_mdb *mdb)
{
	...
	return ocelot_mact_learn(ocelot, mc->pgid->index, addr, vid,
				 mc->entry_type);
}
			...
			const struct switchdev_obj_port_mdb *mdb)
{
	...
	mc->pgid = ocelot_mdb_get_pgid(ocelot, mc);
	if (IS_ERR(mc->pgid))
		return PTR_ERR(mc->pgid);
