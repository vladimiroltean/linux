...
static int sja1105_bridge_vlan_add(struct dsa_switch *ds, int port,
				   ...
				   struct netlink_ext_ack *extack)
{
	/* Always install bridge VLANs as egress-tagged on CPU port */
	u16 flags = dsa_is_cpu_port(ds, port) ? 0 : vlan->flags;
	struct sja1105_private *priv = ds->priv;
	int rc;
...
static const struct dsa_switch_ops sja1105_switch_ops = {
	...
	.port_bridge_fwd_offload_add = dsa_tag_8021q_bridge_fwd_offload_add,
	.port_bridge_fwd_offload_del = dsa_tag_8021q_bridge_fwd_offload_del,
};
