...
static int felix_tag_8021q_vlan_add_rx(struct dsa_switch *ds, int port,
				       struct dsa_port *cpu_dp, u16 vid)
{
	struct ocelot_vcap_filter *outer_tagging_rule;
	struct ocelot *ocelot = ds->priv;
	int upstream = cpu_dp->index;
				    ...
				    u16 flags)
{
	...
	dsa_switch_for_each_cpu_port(cpu_dp, ds) {
		err = felix_tag_8021q_vlan_add_rx(ds, port, cpu_dp, vid);
		if (err)
			return err;
	}
...
add_tx_failed:
	dsa_switch_for_each_cpu_port(cpu_dp, ds)
		felix_tag_8021q_vlan_del_rx(ds, port, cpu_dp, vid);
...
static int felix_tag_8021q_vlan_del(struct dsa_switch *ds, int port, u16 vid)
{
	...
	dsa_switch_for_each_cpu_port(cpu_dp, ds) {
		err = felix_tag_8021q_vlan_del_rx(ds, port, cpu_dp, vid);
		if (err)
			return err;
	}
...
del_tx_failed:
	dsa_switch_for_each_cpu_port(cpu_dp, ds)
		felix_tag_8021q_vlan_add_rx(ds, port, cpu_dp, vid);
