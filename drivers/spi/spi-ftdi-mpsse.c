...
static int ftdi_spi_init_io(struct spi_controller *ctrl, unsigned int dev_idx)
{
	struct ftdi_spi *priv = spi_controller_get_devdata(ctrl);
	struct platform_device *pdev = priv->pdev;
...
//	int ret2;
	...
	ctrl = spi_alloc_host(&pdev->dev, sizeof(*priv));
	if (!ctrl)
		return -ENOMEM;
	...
	ctrl->num_chipselect = max_cs;
	ctrl->min_speed_hz = 450;
//	ctrl->min_speed_hz = 1000000;
	ctrl->max_speed_hz = 30000000;
//	ctrl->max_speed_hz = 25000000;
	ctrl->bits_per_word_mask = SPI_BPW_MASK(8);
	ctrl->set_cs = ftdi_spi_set_cs;
	ctrl->transfer_one = ftdi_spi_transfer_one;
	ctrl->auto_runtime_pm = false;

	dev_info(dev, "before cs_gpio \n");
	priv->cs_gpios = devm_kcalloc(&ctrl->dev, max_cs, sizeof(desc),
				      GFP_KERNEL);
	if (!priv->cs_gpios) {
...
/*
	priv->dc_gpios = devm_kcalloc(&ctrl->dev, dc, sizeof(desc),
				      GFP_KERNEL);

	priv->reset_gpios = devm_kcalloc(&ctrl->dev, reset, sizeof(desc),
				      GFP_KERNEL);

	priv->interrupts_gpios = devm_kcalloc(&ctrl->dev, interrupts, sizeof(desc),
				      GFP_KERNEL);

	priv->irq_gpios = devm_kcalloc(&ctrl->dev, irq, sizeof(desc),
				      GFP_KERNEL);
*/
	for (i = 0; i < num_cs; i++) {
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "Failed to register spi ctrl\n");
		spi_controller_put(ctrl);
		return ret;
	}
	...
	for (i = 0; i < pd->spi_info_len; i++) {
		...
		ret = ftdi_spi_init_io(ctrl, i);
		if (ret < 0) {
			dev_warn(&pdev->dev, "Can't add slave IO: %d\n", ret);
			continue;
		}
		sdev = spi_new_device(ctrl, &pd->spi_info[i]);
		if (!sdev) {
...
static void ftdi_spi_remove(struct platform_device *pdev)
{
	struct spi_controller *ctrl;
	struct ftdi_spi *priv;
	...
	device_for_each_child(&ctrl->dev, priv, ftdi_spi_slave_release);
