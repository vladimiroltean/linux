...
const struct nla_policy ethnl_preempt_get_policy[ETHTOOL_A_PREEMPT_MAX + 1] = {
...
const struct nla_policy ethnl_preempt_set_policy[ETHTOOL_A_PREEMPT_MAX + 1] = {
	[ETHTOOL_A_PREEMPT_UNSPEC]			= { .type = NLA_REJECT },
	[ETHTOOL_A_PREEMPT_HEADER]			= { .type = NLA_NESTED },
	[ETHTOOL_A_PREEMPT_SUPPORTED]			= { .type = NLA_REJECT },
...
int ethnl_set_preempt(struct sk_buff *skb, struct genl_info *info)
{
	...
	ret = nlmsg_parse(info->nlhdr, GENL_HDRLEN, tb,
			  ETHTOOL_A_PREEMPT_MAX, ethnl_preempt_set_policy,
			  info->extack);
	if (ret < 0)
		return ret;
